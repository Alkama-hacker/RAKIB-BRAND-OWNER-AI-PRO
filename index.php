...
// ==========================================
// 1. PHP API PROXY (BYPASS CORS)
// ==========================================
if (isset($_GET['action']) && $_GET['action'] === 'api') {
    error_reporting(0);
    header('Content-Type: text/plain'); // হোস্টিংয়ের স্প্যাম HTML ব্লক করার জন্য
    
    $ts = round(microtime(true) * 1000);
    $url = "https://draw.ar-lottery01.com/WinGo/WinGo_30S/GetHistoryIssuePage.json?ts=" . $ts;
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; user-select: none; }
        ...
        .logic-label { font-size: 7px; font-weight: 700; color: #ffd700; text-align: center; margin-top: 6px; letter-spacing: 0.5px; text-shadow: 0 0 5px rgba(255, 215, 0, 0.5); min-height: 18px; display: flex; align-items: center; line-height: 1.1; }
                ...
                <div id="hackBox">
                    ...
                    <div class="signal-container">
                        <div class="main-signal" id="signalBorder">
                            ...
                            <div class="logic-label" id="logicOutput">CONNECTING...</div>
                        </div>
    ...
    <script>
        ...
        async function fetchGameData() {
            try {
                ...
                // Step 1: Try Local PHP Proxy
                try {
                    let res1 = await fetch('?action=api');
                    txt = await res1.text();
                    match = txt.match(/eyJ[A-Za-z0-9+/=]+/); // Extract pure base64 (removes hosting ads)
                } catch(e) {}

                // Step 2: Fallback to Public Proxy if Step 1 failed
                if (!match) {
                    let ts = Date.now();
                    let targetUrl = encodeURIComponent(`https://draw.ar-lottery01.com/WinGo/WinGo_30S/GetHistoryIssuePage.json?ts=${ts}`);
                    let proxyUrl = `https://api.allorigins.win/raw?url=${targetUrl}`;
                    
                    let res2 = await fetch(proxyUrl);
                    txt = await res2.text();
                    match = txt.match(/eyJ[A-Za-z0-9+/=]+/); // Extract pure base64
                }

                // Step 3: Decode and process data
                if (match) {
                    let cleanBase64 = match[0];
                    let jsonStr = atob(cleanBase64);
                ...
                document.getElementById('logicOutput').innerText = "RETRYING API...";
            }
        }
