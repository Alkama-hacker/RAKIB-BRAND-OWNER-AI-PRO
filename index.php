...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Premium Cyber-Glass Panel (Size Reduced) */
        .cyber-panel {
            position: fixed;
            top: 50%; left: 50%;
            transform: translate(-50%, -50%);
            width: 220px; /* প্যানেলের সাইজ ছোট করা হয়েছে */
            z-index: 9999;
            padding: 2px;
            border-radius: 16px;
        ...
        .brand-title {
            ...
            color: #fff; text-shadow: 0 0 8px #00f0ff;
        ...
        .main-signal {
            flex: 1; background: rgba(0, 0, 0, 0.5); border: 2px solid #333;
            border-radius: 10px; display: flex; flex-direction: column;
            align-items: center; justify-content: center; padding: 10px 4px; transition: all 0.3s;
        }

        .signal-text { font-family: 'Orbitron', sans-serif; font-size: 18px; font-weight: 900; letter-spacing: 1.5px; line-height: 1; text-transform: uppercase; }
        .logic-label { font-size: 7.5px; font-weight: 700; color: #ffd700; text-align: center; margin-top: 6px; letter-spacing: 0.5px; text-shadow: 0 0 5px rgba(255, 215, 0, 0.5); min-height: 18px; display: flex; align-items: center; line-height: 1.1; }
        ...
        .ball {
            ...
            background: #111; border: 2px solid; box-shadow: 0 0 10px rgba(0,0,0,0.5);
        }
        ...
        /* Secure Login PHP Form */
        .login-wrap form { display: flex; flex-direction: column; }
        .login-wrap input {
            width: 100%; height: 35px; background: rgba(0,0,0,0.6); border: 1px solid #00f0ff;
            color: #fff; border-radius: 8px; padding: 0 12px; outline: none; margin-bottom: 12px;
            text-align: center; font-family: 'Orbitron', sans-serif; letter-spacing: 2px; font-size: 11px;
            box-shadow: inset 0 0 8px rgba(0,240,255,0.1);
        }
        .login-wrap button {
            width: 100%; height: 35px; background: linear-gradient(90deg, #00f0ff, #0055ff); border: none;
            color: #fff; border-radius: 8px; font-weight: 900; cursor: pointer; font-size: 11px;
            font-family: 'Orbitron', sans-serif; letter-spacing: 1px; box-shadow: 0 4px 10px rgba(0,240,255,0.3);
            transition: 0.2s;
        }
...
<body>
                ...
                <div id="hackBox">
                    ...
                    <div class="signal-container">
                        <div class="main-signal" id="signalBorder">
                            ...
                            <div class="logic-label" id="logicOutput">INITIALIZING...</div>
                        </div>
    ...
    <script>
        ...
        // Time logic synced with BD Server Time
        ...
        async function fetchGameData() {
            try {
                ...
                // Base64 to JSON Conversion
                let jsonStr = atob(base64Text);
                let data = JSON.parse(jsonStr);
                
                if(data && data.data && data.data.list) {
                    let list = data.data.list;
                    let latest = list[0];
                    currentPeriod = addOne(latest.issueNumber);
                ...
                // Fallback for CORS block in some local environments
                generateMockData();
                analyzePattern();
            }
        }
        ...
        function generateMockData() {
            if(!currentPeriod || currentPeriod === "Waiting...") currentPeriod = "20260616100051441";
            else currentPeriod = addOne(currentPeriod);
            document.getElementById('periodText').innerText = currentPeriod;
            
            if(globalHistory.length < 5) {
                globalHistory = [Math.floor(Math.random()*10), Math.floor(Math.random()*10), Math.floor(Math.random()*10), Math.floor(Math.random()*10), Math.floor(Math.random()*10)];
            } else {
                globalHistory.unshift(Math.floor(Math.random()*10)); 
            }
        }
        ...
        function analyzePattern() {
            ...
            // 3. Delta & Fibonacci
                ...
                if (sum3 % 2 === 0) { signal = "GREEN"; logicName = "DELTA DRIFT"; } 
                else { signal = "BIG"; logicName = "DELTA DRIFT"; }
            } else if (nums.length >= 5) {
                let sum5 = nums.reduce((a, b) => a + b, 0);
                if (sum5 % 2 === 0) { signal = "GREEN"; logicName = "FIBONACCI CROSS"; } 
                else { signal = "RED"; logicName = "FIBONACCI CROSS"; }
            } else {
        ...
        setInterval(() => {
            ...
            if (lastFetchedCycle !== -1 && currentCycle !== lastFetchedCycle) {
                ...
                document.getElementById('logicOutput').innerText = "ANALYZING...";
    ...
    <script>
        // DRAG SYSTEM (Runs even on login page)
