...
// ==========================================
// 1. API PROXY TO BYPASS CORS ERROR
// ==========================================
if (isset($_GET['action']) && $_GET['action'] === 'api') {
    ...
    // User-Agent পাঠানো হচ্ছে যাতে API ব্লক না করে
    curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Linux; Android 11; SM-T295 Build/RP1A.200720.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/148.0.7778.215 Mobile Safari/537.36');
...
<body>
    ...
    <script>
        ...
        // FETCHING DATA FROM LOCAL PHP PROXY INSTEAD OF DIRECT API (100% CORS FIX)
        async function fetchGameData() {
            try {
                let response = await fetch('?action=api');
                let base64Text = await response.text();
                
                let jsonStr = atob(base64Text.trim());
                ...
                console.log(error);
                document.getElementById('logicOutput').innerText = "RECONNECTING API...";
            }
        }
